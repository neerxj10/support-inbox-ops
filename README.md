...
Baseline scores depend on the model and available API access. Run the baseline command above after setting `OPENAI_API_KEY`; the output is structured JSON containing per-task scores and the overall average. This repository includes the exact deterministic setup needed to reproduce the same scoring procedure.
